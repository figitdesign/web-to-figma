 ...
 * the ratchet can only be moved deliberately. The `fix-discrepancy` guard
 * forbids editing this file in an `oracle-fix` PR — the only allowed way to
 * improve a number is to fix the converter. Changes here are calibration
 * decisions (WS-2.6), reviewed on their own.
 */
